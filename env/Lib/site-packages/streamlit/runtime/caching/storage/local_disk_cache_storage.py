...
entries from disk for a single `@st.cache_data_data` decorated function if `persist="disk"`
...
# Streamlit directory where persisted @st.cache_data_data objects live.
# (This is the same directory that @st.cache_data persisted objects live.
# But @st.cache_data_data uses a different extension, so they don't overlap.)
_CACHE_DIR_NAME: Final = "cache"

# The extension for our persisted @st.cache_data_data objects.
# (`@st.cache_data_data` was originally called `@st.memo`)
...
class LocalDiskCacheStorage(CacheStorage):
    ...
    This is the default cache persistence layer for `@st.cache_data_data`
    """
