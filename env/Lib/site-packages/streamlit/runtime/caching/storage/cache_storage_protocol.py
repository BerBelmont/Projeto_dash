...
each function decorated by `@st.cache_data_data` (for example: ttl, max_entries etc.)
...
values for a single `@st.cache_data_data` decorated function serialized as bytes.

How these classes work together
-------------------------------
- CacheStorageContext : this is a dataclass that contains the parameters from
`@st.cache_data_data` that are passed to the CacheStorageManager.create() method.
...
entries for a single `@st.cache_data_data` decorated function.
...
class CacheStorageContext:
    ...
    function_key: str
        ...
        by `@st.cache_data_data`

    function_display_name: str
        The display name of the function that is decorated by `@st.cache_data_data`
...
class CacheStorage(Protocol):
    ...
    Used to store cached values for a single `@st.cache_data_data` decorated function
    serialized as bytes.
...
class CacheStorageManager(Protocol):
    ...
    def check_context(self, context: CacheStorageContext) -> None:
        ...
        check_context is called only once at the moment of creating `@st.cache_data_data`
