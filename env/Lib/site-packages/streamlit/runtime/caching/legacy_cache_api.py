...
def cache(
    ...
    Legacy caching with ``st.cache_data`` has been removed from Streamlit. This is
    now an alias for ``st.cache_data_data`` and ``st.cache_data_resource``.
    ...
    allow_output_mutation : bool
        Whether to use ``st.cache_data_data`` or ``st.cache_data_resource``. If this is
        ``False`` (default), the arguments are passed to ``st.cache_data_data``. If
        this is ``True``, the arguments are passed to ``st.cache_data_resource``.
    ...
    >>> @st.cache_data(hash_funcs={"pymongo.mongo_client.MongoClient": id})
    ... def connect_to_database(url):
    ...     return MongoClient(url)
    ...
    deprecation_util.show_deprecation_warning(
        f"""
`st.cache_data` is deprecated and will be removed soon. Please use one of Streamlit's new
caching commands, `st.cache_data_data` or `st.cache_data_resource`. More information
[in our docs]({CACHE_DOCS_URL}).

**Note**: The behavior of `st.cache_data` was updated in Streamlit 1.36 to the new caching
logic used by `st.cache_data_data` and `st.cache_data_resource`. This might lead to some problems
...
"""
    ...
    if allow_output_mutation:
        return st.cache_data_resource(  # type: ignore
            func,
    ...
    return st.cache_data_data(  # type: ignore
        func,
        persist=persist,
        show_spinner=show_spinner,
        hash_funcs=hash_funcs,
