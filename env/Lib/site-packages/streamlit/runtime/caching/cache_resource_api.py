...
"""@st.cache_data_resource implementation"""
...
def get_resource_cache_stats_provider() -> CacheStatsProvider:
    """Return the StatsProvider for all @st.cache_data_resource functions."""
    return _resource_caches


class CachedResourceFuncInfo(CachedFuncInfo):
    """Implements the CachedFuncInfo interface for @st.cache_data_resource"""
...
class CacheResourceAPI:
    """Implements the public st.cache_data_resource API: the @st.cache_data_resource decorator,
    and st.cache_data_resource.clear().
    """
    ...
    def _decorator(
        ...
        *,
        ...
        cache with ``st.cache_data_resource.clear()``.

        To cache data, use ``st.cache_data_data`` instead. Learn more about caching at
        ...
        ...     name: str
        >>>
        >>> @st.cache_data_resource(hash_funcs={Person: str})
        ...
        ...     name: str
        >>>
        >>> @st.cache_data_resource(hash_funcs={"__main__.Person": str})
        ... def get_person_name(person: Person):
        ...     return person.name
        """
        ...
        # @st.cache_data_resource(show_spinner=False)
        if func is None:
...
class ResourceCache(Cache):
    """Manages cached values for a single st.cache_data_resource function."""
