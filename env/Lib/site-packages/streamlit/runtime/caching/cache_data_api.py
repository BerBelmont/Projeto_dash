...
"""@st.cache_data_data: pickle-based caching"""
...
class CachedDataFuncInfo(CachedFuncInfo):
    """Implements the CachedFuncInfo interface for @st.cache_data_data"""
    ...
    def validate_params(self) -> None:
        """
        Validate the params passed to @st.cache_data_data are compatible with cache storage
...
def get_data_cache_stats_provider() -> CacheStatsProvider:
    """Return the StatsProvider for all @st.cache_data_data functions."""
    return _data_caches


class CacheDataAPI:
    """Implements the public st.cache_data_data API: the @st.cache_data_data decorator, and
    st.cache_data_data.clear().
    """
    ...
    def _decorator(
        ...
        *,
        ...
        cache with ``st.cache_data_data.clear()``.

        To cache global resources, use ``st.cache_data_resource`` instead. Learn more
        ...
        >>> import streamlit as st
        >>>
        >>> @st.cache_data_data(persist="disk")
        ... def fetch_and_clean_data(url):
        ...     # Fetch data from URL here, and then clean it up.
        ...     return data
        ...
        >>> import datetime
        >>>
        >>> @st.cache_data_data(hash_funcs={datetime.datetime: lambda dt: dt.isoformat()})
...
class DataCache(Cache):
    """Manages cached values for a single st.cache_data_data function."""
